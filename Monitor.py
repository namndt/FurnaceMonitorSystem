...
def ConfigLogger():
    LOG_FILE_NAME = 'Furnace_Monitor_Log.txt'
    ...
    LOGGING_CONFIG = {
        ...
        'formatters': {
            'standard': {
                'format': '%(asctime)s [%(levelname)5s] %(thread)5d %(filename)-11s|%(funcName)15s() : %(message)s'
            },
        },
        'handlers': {
            'default_handler': {
...
class INIConfiguration():
    INI_FILE_NAME = 'Furnace_Monitor_Settings.ini'
    application_path = Path(sys.executable).parent
    ...
    def create_ini_file(self):
        ...
        self.__config['LINE_NOTIFY'] = {
            'line_token': '20yBTkYFBYUjp2OSxQI57KuLbuF4LEQd0jYGx2Wkkyy',
            'line_api_url': 'http://10.199.15.109:8080/api/line/notify.php'
        }
        self.__config['FCEL2SV1'] = {
            'ilo_host': '172.22.148.75',
            'ilo_username': 'admin',
            'ilo_password': '12345678',
            'over_temperature_percent': 80,
            'history_database_location': Path.cwd(),
            'monitoring_history_file_name': 'FCEL2SV1_Monitor_History.db',
        }
        self.__config['MSA_STORAGE'] = {
            'msa_host': '172.22.148.76',
            'msa_username': 'manage',
            'msa_password': '!manage',
            'history_database_location': Path.cwd(),
...
class LineNotifyThread(Thread):
    ...
    def send_notify(self, message: str = None):
        ...
        data = {'token': self.LINE_TOKEN, 'message': __inMsg}
        try:
...
class HPiLO_Monitor():
    def __init__(self, server: str) -> None:
        ...
        self.dbapi = SQLiteDB(db_dir=self.DB_DIR, db_name=self.DB_NAME, component='SERVER')

    def apply_config(self, server: str):
        try:
            if server == 'FCEL2SV1':
                self.ILO_HOST = iniconfig.read('FCEL2SV1', 'ilo_host')
                self.ILO_LOGIN = iniconfig.read('FCEL2SV1', 'ilo_username')
                self.ILO_PASSWORD = iniconfig.read('FCEL2SV1', 'ilo_password')
                self.OVER_TEMP_PERCENT = float(iniconfig.read('FCEL2SV1', 'over_temperature_percent'))
                self.DB_DIR = iniconfig.read('FCEL2SV1', 'history_database_location')
                self.DB_NAME = iniconfig.read('FCEL2SV1', 'monitoring_history_file_name')

            logging.debug('=' * 80)
            logging.debug(f'ILO_SERVER HOST [{self.ILO_HOST}] AUTHENTICATION [{self.ILO_LOGIN}@{self.ILO_PASSWORD}]')
    ...
    def get_server_health(self):
        server_health_dict = {}
        try:
            logging.info(f'Connecting to iLO Server management, host {self.ILO_HOST}.')
            ...
            throw_exc = traceback.extract_tb(exc_tb)[0]
            logging.error(f'{exc_type.__name__} in {throw_exc} cause {exc_val}')
            self.__th_network.send_notify(f'[FurnaceMonitor]連不到iLO地址{self.ILO_HOST}，請查看IP地址、帳號或密碼。')
            return
        ...
        try:
            ...
            if fans_err.__len__() != 0:  # There is/are fan(s) failed
                ...
                if self.__th_fan.is_alive():
                    ...
                    self.__th_fan.send_notify()
        except Exception:
            exc_type, exc_val, exc_tb = sys.exc_info()
            throw_exc = traceback.extract_tb(exc_tb)[0]
            logging.error(f'{exc_type.__name__} in {throw_exc} cause {exc_val}')
            self.__th_fan.send_notify(f'[FurnaceMonitor]冷風扇監控失敗，請查看Log檔。')
            return
    ...
    def temperature_EWS(self, temp_data: dict):
        ...
        try:
            ...
            throw_exc = traceback.extract_tb(exc_tb)[0]
            logging.error(f'{exc_type.__name__} in {throw_exc} cause {exc_val}')
            self.__th_temp_error.send_notify('[FurnaceMonitor]溫度感應器監控失敗，請查看Log檔。')
            return
        ...
        if error_sensors.__len__() != 0:
            ...
            if self.__th_temp_error.is_alive():
                ...
                self.__th_temp_error.send_notify()
        # Over temparature
        ...
        try:
            ...
            if net_down.__len__() != 0:
                ...
                if self.__th_network.is_alive():
                    ...
                    self.__th_network.send_notify()
        except Exception:
            exc_type, exc_val, exc_tb = sys.exc_info()
            throw_exc = traceback.extract_tb(exc_tb)[0]
            logging.error(f'{exc_type.__name__} in {throw_exc} cause {exc_val}')
            self.__th_network.send_notify('[FurnaceMonitor]Server Network Adapter監控失敗，請查看Log檔。')
            return
    ...
    def storage_EWS(self, storage_data: dict):
        ...
        try:
            ...
            throw_exc = traceback.extract_tb(exc_tb)[0]
            logging.error(f'{exc_type.__name__} in {throw_exc} cause {exc_val}')
            self.__th_controller.send_notify('[FurnaceMonitor]Server硬碟監控失敗，請查看Log檔。')
            return
        if controller.__len__() != 0:
            ...
            if self.__th_controller.is_alive():
                ...
                self.__th_controller.send_notify()
...
class HPMSA_Monitor():
    ...
    def query_xmlapi(self, url: str, sessionkey):
        ...
        try:
            ...
            throw_exc = traceback.extract_tb(exc_tb)[0]
            logging.error(f'{exc_type.__name__} in {throw_exc} cause {exc_val}')
            self.__generic_line.send_notify(f'[FurnaceMonitor]連不到{full_url}，請查看IP地址、帳號或密碼。')
        ...
        try:
            ...
            if return_code == '0' or return_type == '0':
                ...
                self.__generic_line.send_notify(f'[FurnaceMonitor]調用API接口失敗{full_url}，原因為{return_response}')
    ...
    def phy_disk_EWS(self, properties: List[ET.Element]):
        ...
        try:
            ...
            throw_exc = traceback.extract_tb(exc_tb)[0]
            logging.error(f'{exc_type.__name__} in {throw_exc} cause {exc_val}')
            self.__th_disk.send_notify('[FurnaceMonitor]硬碟監控失敗，請查看Log檔。')
        ...
        try:
            for PROP in properties:
                ...
                if status != 'FTOL' or health != 'OK':
                    ...
                    self.dbapi.execute_sql(
                        "INSERT INTO VDISK (SAMPLING_TIME, NAME, OWNER, STATUS, HEALTH, HEALTH_REASON, RAID_TYPE, CURRENT_JOB, DISK_COUNT, CREATE_DATE, IS_ALARMED) "
                        f"VALUES('{sampling_time}','{name}','{owner}','{status}','{health}','{health_reason}','{raid_type}','{current_job}','{disk_count}','{create_date}',1)"
                    )
                else:
                    self.dbapi.execute_sql(
                        "INSERT INTO VDISK (SAMPLING_TIME, NAME, OWNER, STATUS, HEALTH, HEALTH_REASON, RAID_TYPE, CURRENT_JOB, DISK_COUNT, CREATE_DATE, IS_ALARMED) "
                        f"VALUES('{sampling_time}','{name}','{owner}','{status}','{health}','{health_reason}','{raid_type}','{current_job}','{disk_count}','{create_date}',0)"
                    )
        except Exception:
            exc_type, exc_val, exc_tb = sys.exc_info()
            throw_exc = traceback.extract_tb(exc_tb)[0]
            logging.error(f'{exc_type.__name__} in {throw_exc} cause {exc_val}')
            self.__th_vdisk.send_notify('[FurnaceMonitor]Storage虛擬磁盤監控失敗，請查看Log檔。')
    ...
    def controllers_EWS(self, properties: List[ET.Element]):
        ...
        try:
            ...
            throw_exc = traceback.extract_tb(exc_tb)[0]
            logging.error(f'{exc_type.__name__} in {throw_exc} cause {exc_val}')
            self.__th_control.send_notify('[FurnaceMonitor]Storage控制器監控失敗，請查看Log檔。')
    ...
    def power_EWS(self, properties: List[ET.Element]):
        ...
        try:
            ...
            throw_exc = traceback.extract_tb(exc_tb)[0]
            logging.error(f'{exc_type.__name__} in {throw_exc} cause {exc_val}')
            self.__th_power.send_notify('[FurnaceMonitor]Storage電源監控失敗，請查看Log檔。')
...
class FCEService(ServiceBase):
    # Define service information
    _svc_name_ = 'FurnaceMonitor'
    _svc_display_name_ = 'HSM Furnace Health Monitor'
    _svc_description_ = 'Monitoring iLO server and storage of HSM Furnace system, warning notify with LINE Notify.'
    ...
    def start(self):
        ...
        self._mailer.send_notify(f'熱軋加熱爐{self._svc_name_}服務已啟動成功')
        logging.debug('=' * 80)

    def stop(self):
        self._is_service_run = False
        logging.debug('=' * 80)
        self._mailer.send_notify(f'熱軋加熱爐{self._svc_name_}服務被關閉。請知悉！')
    ...
    def main(self):
        try:
            startTime = time()
            ilo_furnace = HPiLO_Monitor(server='FCEL2SV1')
            msa = HPMSA_Monitor()
            while self._is_service_run:
                ilo_furnace.get_server_health()
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 1:  # by pass error 1503
        ...
        servicemanager.PrepareToHostSingle(FCEService)
        servicemanager.StartServiceCtrlDispatcher()
    else:
        FCEService.parse_command_line(FCEService)
